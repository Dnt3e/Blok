...
for f in SESSION_DIR.glob("session-*"):
    try: L.load_session_from_file(str(f))
    except Exception as e: logging.warning(f"Failed to load session {f}: {e}")
...
def ensure(uid):
    if uid not in users:
        users[uid] = {"role":"admin" if uid==ADMIN_ID else "user", "blocked":False, "accounts":[], "language":None,"interval":1}
        save()
...
async def send_file(p, chat, ctx):
    ...
    except Exception as e: logging.warning(f"Failed to send {p}: {e}")
    finally: os.remove(p)

# ---------- Language ----------
LANG={
    "fa":{"start":"🤖 ربات آماده است","add":"➕ افزودن اکانت","fetch":"⬇️ بررسی جدیدها","link":"🔗 دانلود با لینک",
          "login_required":"❌ برای استوری باید session آپلود شود","back":"🔙 بازگشت","added":"✅ اضافه شد",
          "send_username":"لطفا نام کاربری اینستاگرام را ارسال کنید","send_link":"لطفا لینک را ارسال کنید"},
    "en":{"start":"🤖 Bot is ready","add":"➕ Add Account","fetch":"⬇️ Check New","link":"🔗 Download by Link",
          "login_required":"❌ Instagram session required for stories","back":"🔙 Back","added":"✅ Added",
          "send_username":"Send Instagram username","send_link":"Send link"}
}
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in users or users[uid]["language"] is None:
        keyboard = [[InlineKeyboardButton("🇬🇧 English", callback_data="lang_en")],
                    [InlineKeyboardButton("🇮🇷 فارسی", callback_data="lang_fa")]]
        await update.message.reply_text("Please choose your language / لطفا زبان خود را انتخاب کنید:", 
    ...
    await update.message.reply_text(f"{LANG[lang]['start']}, {update.effective_user.first_name}", 
                                    reply_markup=menu(admin(uid), lang))
...
async def cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith("lang_"):
        ...
        save()
        await q.edit_message_text(f"Language set to {'English' if lang_code=='en' else 'فارسی'}")
        await q.message.reply_text(f"{LANG[lang_code]['start']}, {q.from_user.first_name}", 
                                   reply_markup=menu(admin(uid), lang_code))
        return

    lang = users[uid]["language"]
    if q.data=="add":
        ctx.user_data["await"]="add"
        await q.edit_message_text(LANG[lang]["send_username"])
    elif q.data=="fetch":
        await q.edit_message_text("Checking...")
        for a in users[uid]["accounts"]: await fetch_account(a, q.message.chat_id, ctx)
        await q.edit_message_text("Done", reply_markup=menu(admin(uid), lang))
    elif q.data=="link":
        ctx.user_data["await"]="link"
        await q.edit_message_text(LANG[lang]["send_link"])
...
async def text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if blocked(uid): return
    if users[uid]["language"] is None: return
    lang = users[uid]["language"]

    if ctx.user_data.get("await")=="add":
        users[uid]["accounts"].append(update.message.text.replace("@",""))
        save()
        ctx.user_data["await"]=None
        await update.message.reply_text(LANG[lang]["added"], reply_markup=menu(admin(uid), lang))
    elif ctx.user_data.get("await")=="link":
        ctx.user_data["await"]=None
        await fetch_link(update.message.text, update.message.chat_id, ctx)
        await update.message.reply_text("Done", reply_markup=menu(admin(uid), lang))
...
async def receive_session(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.startswith("session-"): await update.message.reply_text("❌ Invalid session file name"); return
    path=SESSION_DIR/doc.file_name
    file=await doc.get_file()
    await file.download_to_drive(custom_path=str(path))
    try:
        L.load_session_from_file(str(path))
        ctx.user_data["await"]=None
...
async def fetch_link(url, chat_id, ctx):
    ...
    try:
        if "/p/" in url or "/reel/" in url:
            ...
            if not L.context.is_logged_in: await ctx.bot.send_message(chat_id,"Login required"); return
        ...
        else: await ctx.bot.send_message(chat_id,"Unsupported link"); return
        sent=False
...
async def scheduled_fetch():
    for uid,u in users.items():
        if blocked(uid): continue
        for acc in u["accounts"]: await fetch_account(acc, int(uid), app)
...
pip install python-telegram-bot==22.3 instaloader apscheduler
...
ExecStart=$PROJECT/venv/bin/python $PROJECT/telegram_instabot.py
Restart=always
RestartSec=5
