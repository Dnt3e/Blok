...
# ---------- Language ----------
LANG={
    "fa":{"start":"🤖 ربات آماده است","add":"➕ افزودن اکانت","fetch":"⬇️ بررسی جدیدها","link":"🔗 دانلود با لینک","login_required":"❌ برای استوری باید session آپلود شود","back":"🔙 بازگشت","added":"✅ اضافه شد","send_username":"لطفا نام کاربری اینستاگرام را ارسال کنید","send_link":"لطفا لینک را ارسال کنید"},
    "en":{"start":"🤖 Bot is ready","add":"➕ Add Account","fetch":"⬇️ Check New","link":"🔗 Download by Link","login_required":"❌ Instagram session required for stories","back":"🔙 Back","added":"✅ Added","send_username":"Send Instagram username","send_link":"Send link"}
}
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"{LANG[lang]['start']}, {update.effective_user.first_name}", reply_markup=menu(admin(uid), lang))
...
async def text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("await")=="add":
        ...
        await update.message.reply_text(LANG[users[uid]["language"]]["added"], reply_markup=menu(admin(uid), users[uid]["language"]))
